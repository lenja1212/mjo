...
def interpolate_spatial_grid_to_original(olr: OLRData) -> OLRData:
    ...
    * Longitude: Whole globe with 2.5 deg-sampling.
    ...
    print("interpolate_spatial_grid_to_original")
...
def interpolate_spatial_grid(olr: OLRData, target_lat: np.ndarray, target_long: np.ndarray) -> OLRData:
    ...
    """
    print("interpolate_spatial_grid")
...
def restrict_time_coverage_olr(olr_file, time, start: np.datetime64, stop: np.datetime64):
    print("time: ", time)
    window_inds = (time >= start) & (time <= stop)
    if np.all(window_inds == False):  # noqa: E712
        raise ValueError("No olr data within specified period found.")
        #raise ValueError("No olr data within specified period found. Data covers the period from %s to %s."
                         #% (str(olr.time[0]), str(olr.time[-1])))
    else:
        f = netcdf4.Dataset(olr_file, "r")
        #u200_restricted = (f.variables['u'][:].data)[window_inds, :, :]
        olr_restricted = f.variables['olr'][window_inds, :, :]
        #print("u200_restricted",u200_restricted)
        return olr_restricted, time[window_inds] 
...
def load_noaa_interpolated_olr(filename: Path) -> OLRData:
    ...
    :return: The OLR data.
    """
    print("load_noaa_interpolated_olr")
...
def load_noaa_interpolated_olr_netcdf4(filename: Path, start: np.datetime64, stop: np.datetime64, plav=False) -> OLRData:
    """
    ...
    :return: The OLR data.
    """
    print("load_noaa_interpolated_olr_netcdf4")
    f = netcdf4.Dataset(filename, "r")
    lat = f.variables['lat'][:].data.copy()
    lon = f.variables['lon'][:].data.copy()
    #olr = abs(f.variables['olr'][:]).data.copy()
    #olr = f.variables['olr'][:].data.copy()
    hours_since1800 = f.variables['time'][:].data.copy()
    #print("hours_since1800: ", hours_since1800)
    #exit()
    f.close()
    ...
    olr_restricted, time_restricted = restrict_time_coverage_olr( filename, time , start, stop)
    print("len_time_after:", len(time_restricted))
    #print("time_restricted", time_restricted)
    #exit()
    # return olr_restricted
    result = OLRData(np.squeeze(olr_restricted), time_restricted, lat, lon)
    return result

#def load_noaa_interpolated_olr_120d_before_netcdf4(filename: Path, start: np.datetime64, stop: np.datetime64) -> OLRData:
    #day_start = start - np.timedelta64(120, 'D') 
    #result = load_noaa_interpolated_olr_netcdf4(filename, day_start, stop)
    #get_averege_array_120d_before(result.olr)

#def get_averege_array_120d_before(olr_array):
    #average_olr_arr = []

    #for i in range(len(olr_array)):    
        #olr_arr = olr_array[i : 120+i].flatten()
        #print("olr_arr: ",olr_arr.tolist())
        #print("olr_arr: ", len(olr_arr))
        #print("olr_arr shape: ", olr_arr)
        #non_zero = np.count_nonzero(olr_arr)
        #print("Sum: ",sum(olr_arr)/non_zero)
        #exit()
        
def get_average_olr_for_day(filename: Path , current_day, current_month , current_year):
    temptime = []
    dates = []
    olr_at_day = []
    f = netcdf4.Dataset(filename, "r")
    ...
    for i in range(len(time_all)-1):
        if time_all[i] in dates:
            olr_at_day.append(f.variables['olr'][i])
    f.close()
    average = sum(olr_at_day) / len(dates)
    return average


def get_doys_in_span(olr: OLRData, filename: Path, start:np.datetime64, stop:np.datetime64):
    ...
    for i in range(len(days)):
        print("Find olr anomaly for date: ", dates[i])
        average = get_average_olr_for_day(filename, days[i].astype(int), months[i].astype(int), years[i])
        get_doy_avarage(olr, average, dates[i])
        
    return OLRData(olr.olr, olr.time, olr.lat, olr.long)
    
def get_doy_avarage(olr: OLRData, average, doy):
    for i in range(len(olr.time)-1):
        if olr.time[i] == doy:
            olr.olr[i] -= average
            break
